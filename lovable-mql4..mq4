...
extern int    InpTrailingStructureLookback = 10;
...
void CheckForClosedTrades()
{
   ...
   int liveTicket = FindOurTicket();

   if(g_msActive == 1)
   {
      if(liveTicket > 0)
      {
         if(currentClosedCount > g_previousClosedCount)
         {
            LogDecisionEvent("TRADE_EVENT", "PARTIAL_CLOSE_DETECTED historyDelta=" + IntegerToString(currentClosedCount - g_previousClosedCount), g_msCurrentR);
         }
         if(g_msTicket != liveTicket)
         {
            LogDecisionEvent("TRADE_EVENT", "ROLLOVER_TICKET old=" + IntegerToString(g_msTicket) + " new=" + IntegerToString(liveTicket), g_msCurrentR);
            g_msTicket = liveTicket;
         }
      ...
      for(int i = OrdersHistoryTotal()-1; i >= 0; i--)
      {
         ...
         if(OrderSymbol() != InpSymbolName) continue;
         if(OrderMagicNumber() != InpMagicNumber) continue;
         finalProfit = OrderProfit() + OrderSwap() + OrderCommission();
         finalFound = true;
         break;
      }
      if(finalFound && finalProfit < 0)
      {
         g_dailyLossCount++;
         g_weeklyLossCount++;
      }
      LogDecisionEvent("TRADE_EVENT", "FULL_CLOSE_DETECTED ticket=" + IntegerToString(g_msTicket), g_msCurrentR);
...
int GetWeekStamp(MqlDateTime dt)
{
...
bool PassVWAPBias(int side, string stage)
{
   ...
   LogDecisionEvent(stage, "VWAP " + (pass ? "PASS" : "FAIL") + " px=" + DoubleToString(px,Digits) + " vwap=" + DoubleToString(vwap,Digits), 0);
   return pass;
}
...
int DetectRegime()
{
   ...
   if(atrNow > 0)
   {
      ...
      for(int i = 1; i <= 100; i++)
      {
         ...
         histCount++;
         if(atrVal <= atrNow) countBelow++;
      }
      if(histCount > 0) atrPercentile = (double)countBelow / (double)histCount * 100.0;
   }
...
void EvaluateEntry()
{
   ...
   if(EnableOnePositionOnly)
   {
      ...
      {
         LogDecisionEvent("PRECHECK", "One-position policy active; existing position present", 0);
         return;
      }
   }
   else
...
void OpenTrade(int side, int regime)
{
   ...
   if(ticket > 0)
   {
      if(OrderSelect(ticket, SELECT_BY_TICKET))
      {
         ...
         double point = MarketInfo(InpSymbolName, MODE_POINT);
         g_msSlippageAtEntry = (point > 0 && requestPriceUsed > 0) ? MathAbs(OrderOpenPrice() - requestPriceUsed) / point : 0;
         g_msComplianceViolations = 0;
         g_msStopDueSpread = 0;
         g_msStopDueSlippage = 0;
         g_sessionTradeCount++;
         LogDecisionEvent("ORDER_SUBMIT", "Opened " + ((side==1)?"BUY":"SELL") +
            ...
            " spread=" + DoubleToString(spreadAtEntry,1) +
            " slippage=" + DoubleToString(g_msSlippageAtEntry,1), 0);
         LogDecisionEvent("ORDER_FILL", "POST_FILL_SLIPPAGE points=" + DoubleToString(g_msSlippageAtEntry,1) +
            " requested=" + DoubleToString(requestPriceUsed,Digits) + " fill=" + DoubleToString(OrderOpenPrice(),Digits), 0);
      }
   }
   else
   {
...
void ApplyTrailingLogic()
{
   ...
   // Swing Trail (protective structural pivot)
   if(method == CM_SWING && EnableSwingTrail)
   {
      if(g_msDirection == 1)
      {
         double pivotLow = iLow(InpSymbolName, InpExecTF, 1);
         for(int i = 2; i <= trailLookback; i++)
         {
            double lo = iLow(InpSymbolName, InpExecTF, i);
            if(lo < pivotLow) pivotLow = lo;
         }
         double targetSL = pivotLow - atr * 0.2;
         LogDecisionEvent("MANAGEMENT", "SWING_TRAIL pivotLow=" + DoubleToString(pivotLow,Digits) + " targetSL=" + DoubleToString(targetSL,Digits), g_msCurrentR);
         MoveSL(targetSL, "SWING_TRAIL");
      }
      else
      {
         double pivotHigh = iHigh(InpSymbolName, InpExecTF, 1);
         for(int i = 2; i <= trailLookback; i++)
         {
            double hi = iHigh(InpSymbolName, InpExecTF, i);
            if(hi > pivotHigh) pivotHigh = hi;
         }
         double targetSL = pivotHigh + atr * 0.2;
         LogDecisionEvent("MANAGEMENT", "SWING_TRAIL pivotHigh=" + DoubleToString(pivotHigh,Digits) + " targetSL=" + DoubleToString(targetSL,Digits), g_msCurrentR);
         MoveSL(targetSL, "SWING_TRAIL");
      }
   }
   // Fast Bank
   ...
   if(method == CM_HYBRID && EnableHybridCloseMode)
   {
      if(g_msDirection == 1)
      {
         ...
         double structSL = pivotLow - atr * 0.2;
         double finalSL = MathMax(atrSL, structSL);
         LogDecisionEvent("MANAGEMENT", "HYBRID_TRAIL pivotLow=" + DoubleToString(pivotLow,Digits) + " atrSL=" + DoubleToString(atrSL,Digits) + " finalSL=" + DoubleToString(finalSL,Digits), g_msCurrentR);
         MoveSL(finalSL, "HYBRID_TRAIL");
      }
      else
      {
         double atrSL = currentPrice + atr * InpATRTrailMultiplier;
         ...
         double structSL = pivotHigh + atr * 0.2;
         double finalSL = MathMin(atrSL, structSL);
         LogDecisionEvent("MANAGEMENT", "HYBRID_TRAIL pivotHigh=" + DoubleToString(pivotHigh,Digits) + " atrSL=" + DoubleToString(atrSL,Digits) + " finalSL=" + DoubleToString(finalSL,Digits), g_msCurrentR);
         MoveSL(finalSL, "HYBRID_TRAIL");
      }
   }
}
